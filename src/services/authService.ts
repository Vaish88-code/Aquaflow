...
export const authService = {
  ...
  async sendOTP(phoneNumber: string, pincode?: string): Promise<AuthResponse> {
    try {
      ...
      try {
        const response = await fetch(`${((import.meta as any).env?.VITE_API_BASE_URL) || 'http://localhost:5000/api'}/users/send-otp`, {
          method: 'POST',
          headers: {
  ...
  async verifyOTPAndLogin(phoneNumber: string, otp: string): Promise<AuthResponse> {
    try {
      ...
      try {
        const response = await fetch(`${((import.meta as any).env?.VITE_API_BASE_URL) || 'http://localhost:5000/api'}/users/verify-otp`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  async getNearbyShops(userPincode?: string, userState?: string): Promise<any> {
    try {
      ...
      try {
        const response = await fetch(`${((import.meta as any).env?.VITE_API_BASE_URL) || 'http://localhost:5000/api'}/users/shops/by-pincode?pincode=${userPincode}`);
